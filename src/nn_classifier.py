...
# function that loads the mnist_784 dataset
def load_mnist():
...
# function that scales data with min and max values
def minmax_scaling(data):
    X_norm = (data - data.min()) / (data.max() - data.min())
    return X_norm


# function that splits data in train and test split
def split_data(X, y):
...
# function that loads the cifar10 data
def load_cifar10():
...
# function that fits the neural network
...
def main():
    ...
    if data_set == 'mnist_784':
        ...
        X_train_dataset, X_test_dataset, y_train, y_test = load_cifar10()
        print(y_train.shape)
